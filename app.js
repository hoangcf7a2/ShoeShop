...
app.use((req,res,next)=>{
  // Req.headers.origin là lấy domain của req,nếu domain của request đc cho phép thì được set header access control allow origin
  if (
    process.env.ALLOW_HOST_ORIGIN.split(",").indexOf(req.headers.origin) !== -1
  ) {
  ...
  ); // cho phép gửi request kèm với header
